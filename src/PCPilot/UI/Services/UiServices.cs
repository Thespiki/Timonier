...
/// <summary>
/// Raisons pour lesquelles PC Pilot doit rester actif quand on ferme la fenêtre (accès guidé actif, tâche en cours…).
...
public static class SystemEffects
{
    ...
    public static Task RebootNowAsync() =>
        ProcessRunner.RunAsync(SystemTool.Shutdown, ["/r", "/t", "5", "/c", "Redémarrage demandé depuis PC Pilot"], new RunOptions());

    public static Task SignOutNowAsync() =>
        ProcessRunner.RunAsync(SystemTool.Shutdown, ["/l"], new RunOptions());
}

/// <summary>Icône de la zone de notification (créée uniquement si PC Pilot reste actif en arrière-plan).</summary>
public sealed class TrayIcon : IDisposable
{
    ...
    public void Show(IReadOnlyList<string> reasons)
    {
        if (_icon is null)
        {
            ...
            _icon.ContextMenuStrip.Items.Add("Ouvrir PC Pilot", null, (_, _) => _open());
            _icon.ContextMenuStrip.Items.Add("Quitter", null, (_, _) => _exit());
            _icon.DoubleClick += (_, _) => _open();
        }
        var text = reasons.Count == 0 ? "PC Pilot" : "PC Pilot — " + string.Join(", ", reasons);
