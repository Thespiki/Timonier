...
internal sealed class GuidedSession : IDisposable
{
    ...
    private unsafe void Begin()
    {
        ...
        // Masquage de PC Pilot après le traitement de l'événement « arrière-plan » (priorité inférieure) : la fenêtre est
    ...
    public void End(bool showSummary = true)
    {
        ...
        try { (Application.Current as App)?.ShowMainWindow(); } catch (Exception ex) { Log.Error("GuidedAccess", "réaffichage de PC Pilot", ex); }
        _background?.Dispose();
        _background = null;
