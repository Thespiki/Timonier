...
internal sealed class ApplyKioskAction : IActionHandler
{
    ...
    private const string SetAssignedAccessScript =
        "Import-Module AssignedAccess; Set-AssignedAccess -AppUserModelId $env:PCP_AUMID -UserSID $env:PCP_SID -Confirm:$false; 'OK'";
    ...
    public async Task<ActionResult> ExecuteAsync(ActionContext ctx, IReadOnlyDictionary<string, string> p)
    {
        ...
        // 2. Accès attribué : configuré pour une application du Store, retiré si PC Pilot l'avait posé et qu'on change de mode.
        var assigned = false;
...
/// <summary>Retire tout ce que PC Pilot a configuré pour la borne (et, sur demande, l'accès attribué et l'ouverture automatique).</summary>
            ...
            try
            {
                ...
                else
                {
                    if (state.ShellValue is not null)
                    {
                        ...
                        // On ne touche à la valeur que si elle est toujours celle posée par PC Pilot.
