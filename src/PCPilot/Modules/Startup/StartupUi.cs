...
internal static class StartupUi
{
    ...
    public static void OpenConsole(string msc)
    {
        ...
        catch (Exception ex)
        {
            ...
            AppHost.Toasts.Show("Impossible d'ouvrir la console : " + ex.Message, UI.Services.ToastKind.Error);
        }
    }
    ...
    public static void RevealInExplorer(string path)
    {
        ...
        {
            AppHost.Toasts.Show("Impossible d'ouvrir l'emplacement : " + ex.Message, UI.Services.ToastKind.Error);
        }
    }
}
...
internal sealed class PagedList<T> : StackPanel
{
    ...
    public PagedList(Func<T, FrameworkElement> factory, int pageSize = 40)
    {
        _factory = factory;
        _pageSize = pageSize;
        _more = StartupUi.Button("Afficher plus", "", "Pp.Button", (_, _) => ShowMore());
    ...
    private void ShowMore()
    {
        ...
        System.Windows.Automation.AutomationProperties.SetName(_more, $"Afficher plus ({remaining} restants)");
        if (_more.Content is StackPanel p && p.Children.Count > 1 && p.Children[1] is TextBlock t)
            t.Text = $"Afficher plus ({remaining} restant{(remaining > 1 ? "s" : "")})";
    }
}
