...
internal static class AccountParams
{
    ...
    public const string ConfirmTitle = "PC Pilot — confirmation administrateur";
...
public sealed class CreateAccountAction : IActionHandler
{
    ...
    public Task<ActionResult> ExecuteAsync(ActionContext ctx, IReadOnlyDictionary<string, string> p)
    {
        ...
        if (admin && ctx.Elevated && !PcPilot.Core.Platform.Native.ConfirmFromElevatedProcess(AccountParams.ConfirmTitle,
...
public sealed class SetAccountEnabledAction : IActionHandler
{
    ...
    public Task<ActionResult> ExecuteAsync(ActionContext ctx, IReadOnlyDictionary<string, string> p)
    {
        ...
        var (target, all) = LocalAccounts.Resolve(AccountParams.Sid(p), ctx.UserSid);
        if (target.Rid is 503 or 504) throw new ValidationException("Ce compte technique est géré par Windows : PC Pilot ne le modifie pas.");
        if (enable)
        {
            ...
            if ((target.IsBuiltInAdministrator || target.IsGuest) && !target.Enabled)
                throw new ValidationException("Par sécurité, PC Pilot ne réactive pas le compte Administrateur intégré ni le compte Invité.");
        }
        else
        {
...
public sealed class SetAccountTypeAction : IActionHandler
{
    ...
    public Task<ActionResult> ExecuteAsync(ActionContext ctx, IReadOnlyDictionary<string, string> p)
    {
        ...
        if (admin)
        {
            if (ctx.Elevated && !PcPilot.Core.Platform.Native.ConfirmFromElevatedProcess(AccountParams.ConfirmTitle,
...
public sealed class ResetPasswordAction : IActionHandler
{
    ...
    public Task<ActionResult> ExecuteAsync(ActionContext ctx, IReadOnlyDictionary<string, string> p)
    {
        ...
        if (target.Rid is 501 or 503 or 504) throw new ValidationException("Ce compte intégré n'utilise pas de mot de passe géré par PC Pilot.");
