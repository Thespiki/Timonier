...
internal sealed class InventoryPanel : UserControl
{
    ...
    public async Task LoadAsync()
    {
        ...
        catch (Exception ex)
        {
            ...
            _messages.Children.Clear();
            var bar = PcPilot.UI.Controls.PageScaffold.InfoBar(
    ...
    /// <summary>Oublie les périphériques de la liste « Désactivés par PC Pilot » qui ont été réactivés entre-temps (ailleurs).</summary>
    ...
    private async Task RunActionAsync(DeviceEntry d, bool enable, Button button)
    {
        ...
        else
        {
            ...
            var message = $"« {d.Name} » ne fonctionnera plus jusqu'à sa réactivation." +
                          ...
                          "\n\nVous pourrez le réactiver ici, même s'il n'apparaît plus dans la liste (section « Désactivés par PC Pilot »).";
    ...
    private void RenderDisabledStore()
    {
        ...
        head.Children.Add(new TextBlock { Text = "Désactivés par PC Pilot", Margin = new Thickness(10, 0, 0, 0), VerticalAlignment = VerticalAlignment.Center }.Styled("Pp.CardTitle"));
        body.Children.Add(head);
