...
#pragma warning restore SYSLIB1054
    ...
    private static Dictionary<string, string> FriendlyNames()
    {
        ...
        try
        {
            foreach (var row in PcPilot.Core.Platform.WmiQuery.Query("SELECT InstanceName, UserFriendlyName FROM WmiMonitorID", @"root\wmi", 10))
            {
        ...
        {
            PcPilot.Core.Platform.Log.Warn("Devices", "noms des écrans : " + ex.Message);
        }
        return map;
    }
