    ...
    public async Task LoadAsync()
    {
        ...
        if (snap.Error is not null || snap.Radios.Count == 0)
        {
            var message = snap.Error is not null
                ? "PC Pilot ne peut pas lire l'état des radios sur ce PC. Utilisez les Paramètres Windows."
        ...
        if (snap.Access != RadioAccessStatus.Allowed)
        {
            ...
            {
                Text = "Windows n'autorise pas PC Pilot à changer l'état des radios (réglage « Contrôle des radios » de la page Confidentialité) : " +
