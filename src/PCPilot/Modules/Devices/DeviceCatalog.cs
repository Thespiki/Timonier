...
/// <summary>Niveau de protection d'un périphérique vis-à-vis de la désactivation par PC Pilot.</summary>
public enum DeviceProtection
{
    ...
    Sensitive,
    /// <summary>PC Pilot refuse de désactiver ce périphérique (composant indispensable au démarrage ou à la sécurité).</summary>
    Protected,
}
...
public static class DeviceCatalog
{
    ...
    /// <summary>Classes que PC Pilot refuse de désactiver, avec la raison affichée à l'utilisateur.</summary>
    ...
    public static (DeviceProtection Level, string? Reason) Protection(string? pnpClass, string instanceId)
    {
        ...
        if (cls.Equals("DiskDrive", StringComparison.OrdinalIgnoreCase))
        {
            return instanceId.StartsWith(@"USBSTOR\", StringComparison.OrdinalIgnoreCase)
                ...
                : (DeviceProtection.Protected, "Disque interne : PC Pilot ne désactive que les disques USB externes.");
        }
        ...
        if (cls.Length == 0)
            return (DeviceProtection.Sensitive, "Classe inconnue : PC Pilot ne peut pas évaluer le rôle de ce périphérique.");
