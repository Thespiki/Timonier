    ...
    private static List<string> Parse(IReadOnlyDictionary<string, string> p)
    {
        ...
        foreach (var full in list)
        {
            ...
            if (BloatCatalog.IsProtected(BloatCatalog.NameOf(full)))
                throw new ValidationException($"{BloatCatalog.NameOf(full)} est un composant protégé de Windows : PC Pilot ne le supprime pas.");
        }
        return list;
    }
...
public sealed class AppxDeprovisionAction : IActionHandler
{
    ...
    // Script constant : les noms de famille arrivent uniquement par $env:PCP_FAMILIES (déjà validés par liste blanche).
    private const string Script = """
        foreach ($fam in ($env:PCP_FAMILIES -split ',')) {
    ...
    public async Task<ActionResult> ExecuteAsync(ActionContext ctx, IReadOnlyDictionary<string, string> p)
    {
        ...
        var removed = new List<string>();
        var failures = 0;
        foreach (var line in r.Output.Split('\n').Select(l => l.Trim()).Where(l => l.StartsWith("PCP_RESULT|", StringComparison.Ordinal)))
        {
