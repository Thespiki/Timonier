...
internal sealed class BloatView : StackPanel
{
    ...
    public BloatView(AppsContext ctx)
    {
        ...
        Children.Add(AppsUi.InfoBar(
            "Ces applications sont supprimées pour votre compte uniquement, sans droits administrateur, et se réinstallent depuis le " +
            "Microsoft Store. Les composants indispensables de Windows (Store, installateur d'applications, sécurité, interface…) " +
            "sont protégés et n'apparaissent pas ici. Une mise à jour majeure de Windows peut réinstaller certaines applications.", ""));

        _removeSelected = AppsUi.Button("Supprimer la sélection", "", "Pp.AccentButton", async (_, _) => await RemoveSelectedAsync());
        _allUsers = AppsUi.CheckBox("Aussi pour tous les comptes");
        _allUsers.Content = "Aussi pour tous les comptes et les futurs comptes (administrateur)";
        ...
        _allUsers.ToolTip = "Supprime aussi l'application pour les autres comptes et la retire de l'image de Windows, " +
                            "pour qu'elle ne soit plus installée à la création d'un compte.";
        _refresh = AppsUi.Button("Actualiser", "", "Pp.SubtleButton", async (_, _) => await LoadAsync());
    ...
    private async Task LoadAsync()
    {
        ...
        if (!_loaded) _host.Content = AppsUi.Loading("Analyse des applications du Store…");
        try
        {
        ...
        catch (Exception ex)
        {
            ...
            _host.Content = AppsUi.EmptyState("", "Liste indisponible", "Les applications du Store n'ont pas pu être énumérées : " + ex.Message, "Pp.Warning");
        }
        finally
        {
            _loading = false;
    ...
    private void BuildContent(List<AppxInfo> apps)
    {
        ...
        var known = apps.Where(a => a.Bloat is not null && !a.IsSystemSigned)
            ...
            .ToList();
        root.Children.Add(AppsUi.SectionHeader("Applications préinstallées détectées", "", out var knownCounter));
        knownCounter.Text = known.Count.ToString();
        if (known.Count == 0)
        {
            root.Children.Add(AppsUi.EmptyState("", "Rien à nettoyer",
                "Aucune des applications préinstallées superflues connues n'est présente pour votre compte.", "Pp.Success"));
        }
        else
        {
        ...
        if (others.Count > 0)
        {
            ...
            {
                Header = $"Autres applications du Store installées pour votre compte ({others.Count})",
        ...
        if (removed.Count > 0)
        {
            root.Children.Add(AppsUi.SectionHeader("Supprimées avec PC Pilot", "", out var removedCounter));
    ...
    private Border RemovedRow(RemovedApp r)
    {
        ...
        text.Children.Add(AppsUi.Caption($"Supprimée le {r.Date.ToString("d MMMM yyyy", Fr)} · {BloatCatalog.NameOf(r.Family)}"));
        Grid.SetColumn(text, 1);
        grid.Children.Add(text);
        var reinstall = AppsUi.Button("Réinstaller depuis le Store", "", "Pp.Button", (_, _) =>
        {
        ...
        Border? row = null;
        var forget = AppsUi.Button("Retirer", null, "Pp.SubtleButton", (_, _) =>
        {
            AppsContext.ForgetRemoved(r.Family);
            if (row?.Parent is Panel panel) panel.Children.Remove(row);
        });
        forget.ToolTip = "Retirer de cet historique";
    ...
    private void UpdateButtons()
    {
        ...
        AppsUi.SetButtonText(_removeSelected, n == 0 ? "Supprimer la sélection" : $"Supprimer la sélection ({n})");
    ...
    private async Task RemoveAsync(List<AppxInfo> apps)
    {
        ...
        content.Children.Add(AppsUi.Caption(allUsers
            ? "Ces applications seront supprimées pour tous les comptes de ce PC et retirées de l'image de Windows :"
            : "Ces applications seront supprimées pour votre compte :"));
        ...
        content.Children.Add(AppsUi.InfoBar("Vous pourrez les réinstaller depuis le Microsoft Store (historique en bas de cette page).", ""));
        var danger = apps.Any(a => a.Bloat?.Risk == BloatRisk.Moderate || a.Bloat is null);
        if (!await AppHost.Dialogs.ShowAsync(apps.Count == 1 ? "Supprimer cette application ?" : $"Supprimer {apps.Count} applications ?",
                content, "Supprimer", "Annuler", danger)) return;

        var title = apps.Count == 1 ? $"Suppression de {apps[0].Title}" : $"Suppression de {apps.Count} applications";
        var outcome = allUsers
            ? await _ctx.Activity.RunAsync(title + " (tous les comptes)", AppxDeprovisionAction.ActionId,
    ...
    private sealed class BloatRow : Border
    {
        ...
        public BloatRow(AppxInfo app, BloatView owner, bool selectable)
        {
            ...
            if (selectable)
            {
                _check = AppsUi.CheckBox("Sélectionner " + app.Title);
            ...
            if (app.Bloat is { } b)
            {
                top.Children.Add(b.Risk == BloatRisk.Safe ? AppsUi.Badge("Sans risque", "Pp.Success") : AppsUi.Badge("À considérer", "Pp.Warning"));
                if (b.Recommended) top.Children.Add(AppsUi.Badge("Recommandé", "Pp.Accent"));
            }
            text.Children.Add(top);
            var desc = AppsUi.Caption(app.Bloat?.Description ?? (app.Publisher.Length > 0 ? app.Publisher : "Application du Store"));
            ...
            var tech = AppsUi.Caption($"{app.Name} · version {app.Version}");
            ...
            _remove = AppsUi.Button("Supprimer", "", "Pp.Button", async (_, _) => await owner.RemoveAsync([app]));
