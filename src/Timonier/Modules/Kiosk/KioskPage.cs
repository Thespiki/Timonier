...
public sealed partial class KioskPage : UserControl, INavigationAware
{
    ...
    private Border BuildCompatibility()
    {
        ...
        stack.Children.Add(CompatRow("", "Shell Launcher et kiosque multi-applications",
            p.SupportsShellLauncher
                ? "Pris en charge par votre édition, mais configurés par fichier XML (Windows Configuration Designer, MDM/Intune) : Timonier ne les gère pas."
                : "Shell Launcher est réservé aux éditions Entreprise et Éducation ; le kiosque multi-applications nécessite une configuration XML ou MDM. Timonier ne les gère pas.",
            ("Non géré", "Neutral")));
    ...
    private void RenderStatus()
    {
        ...
        {
            var head = Row(null, "Aucune borne configurée par Timonier",
    ...
    private async Task RemoveAsync(Button button)
    {
        ...
        content.Children.Add(Text(s is { IsConfigured: true }
            ? $"Le compte « {s.User} » retrouvera son Bureau Windows à sa prochaine connexion et les restrictions posées par Timonier seront retirées (les valeurs d'origine sont restaurées)."
            : "Timonier n'a pas de borne enregistrée : seules les options cochées ci-dessous seront appliquées."));
        var aa = new CheckBox
        {
