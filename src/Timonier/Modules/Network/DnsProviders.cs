...
internal static class DnsProviders
{
    ...
    [
        new("cloudflare", "Cloudflare",
            "Rapide et sans filtrage. Cloudflare s'engage à ne pas conserver votre adresse IP plus de 25 heures (audits publics).",
            DnsFiltering.None,
            ["1.1.1.1", "1.0.0.1"], ["2606:4700:4700::1111", "2606:4700:4700::1001"],
            "https://cloudflare-dns.com/dns-query"),
        new("cloudflare-family", "Cloudflare Famille",
            "Bloque les sites malveillants et les contenus pour adultes (1.1.1.3). Pensé pour un usage familial ou parental.",
            DnsFiltering.Family,
            ["1.1.1.3", "1.0.0.3"], ["2606:4700:4700::1113", "2606:4700:4700::1003"],
            "https://family.cloudflare-dns.com/dns-query"),
        new("quad9", "Quad9",
            "Fondation suisse à but non lucratif : bloque les domaines malveillants connus (hameçonnage, logiciels malveillants), sans journaliser les adresses IP.",
            DnsFiltering.Malware,
            ["9.9.9.9", "149.112.112.112"], ["2620:fe::fe", "2620:fe::9"],
            "https://dns.quad9.net/dns-query"),
        new("google", "Google Public DNS",
            "Fiable et sans filtrage. Google conserve des journaux temporaires (24 à 48 h) et des données agrégées.",
            DnsFiltering.None,
            ["8.8.8.8", "8.8.4.4"], ["2001:4860:4860::8888", "2001:4860:4860::8844"],
            "https://dns.google/dns-query"),
        new("adguard", "AdGuard DNS",
            "Bloque une grande partie des publicités et des traqueurs pour tous les navigateurs et applications. Peut casser certains sites ou liens sponsorisés.",
            DnsFiltering.AdsTrackers,
            ["94.140.14.14", "94.140.15.15"], ["2a10:50c0::ad1:ff", "2a10:50c0::ad2:ff"],
            "https://dns.adguard-dns.com/dns-query"),
        new("adguard-family", "AdGuard Famille",
            "Publicités, traqueurs et contenus pour adultes bloqués ; impose la recherche sécurisée (Google, Bing, YouTube en mode restreint).",
            DnsFiltering.Family,
            ["94.140.14.15", "94.140.15.16"], ["2a10:50c0::bad1:ff", "2a10:50c0::bad2:ff"],
            "https://family.adguard-dns.com/dns-query"),
        new("opendns-family", "OpenDNS FamilyShield",
            "Service de Cisco qui bloque automatiquement les contenus pour adultes, sans compte à créer.",
            DnsFiltering.Family,
    ...
    {
        DnsFiltering.Family => "Filtre familial",
        DnsFiltering.Malware => "Bloque les menaces",
        DnsFiltering.AdsTrackers => "Bloque pubs et traqueurs",
        _ => "Sans filtrage",
    };
    ...
    public static (List<string> V4, List<string> V6) ParseCustom(string input)
    {
        ...
        if (parts.Length == 0) throw new ValidationException("Saisissez au moins une adresse de serveur DNS.");
        if (parts.Length > 4) throw new ValidationException("Quatre adresses au maximum (2 IPv4 et 2 IPv6).");
        var v4 = new List<string>();
        var v6 = new List<string>();
        foreach (var part in parts)
        {
            if (part.Contains('%')) throw new ValidationException($"Identifiant de zone non accepté : {part}");
            ...
            if (ip.Equals(IPAddress.Any) || ip.Equals(IPAddress.IPv6Any) || ip.Equals(IPAddress.Broadcast) || ip.Equals(IPAddress.None))
                throw new ValidationException($"Adresse non utilisable comme serveur DNS : {part}");
            if (ip.AddressFamily == AddressFamily.InterNetworkV6 ? ip.IsIPv6Multicast : (ip.GetAddressBytes()[0] & 0xF0) == 0xE0)
                throw new ValidationException($"Adresse de multidiffusion refusée : {part}");
            var canonical = ip.ToString();
            var target = ip.AddressFamily == AddressFamily.InterNetwork ? v4 : v6;
            if (target.Contains(canonical, StringComparer.OrdinalIgnoreCase)) throw new ValidationException($"Adresse en double : {part}");
            target.Add(canonical);
        }
        if (v4.Count > 2) throw new ValidationException("Deux adresses IPv4 au maximum.");
        if (v6.Count > 2) throw new ValidationException("Deux adresses IPv6 au maximum.");
        return (v4, v6);
    }
}
