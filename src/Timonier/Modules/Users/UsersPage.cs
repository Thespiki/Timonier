...
public sealed class UsersPage : UserControl, INavigationAware
{
    ...
    private readonly TextBlock _lockoutState = Caption(L("Reading policy…"));
    ...
    public UsersPage()
    {
        ...
        {
            Title = L("Users and parental controls"),
            Subtitle = L("Accounts on this PC, sign-in hours and restrictions for children, sign-in screen."),
        ...
        stack.Children.Add(SectionHeader(L("Microsoft parental controls"), out _familyHeading));
        stack.Children.Add(BuildFamilyCard());

        stack.Children.Add(SectionHeader(L("Sign-in and lockout"), out _signInHeading));
        _lockoutApply = MakeButton(L("Apply"), GlyphAdmin, "Pp.Button", async (_, _) => await ApplyLockoutAsync());
    ...
    private Border BuildSummary()
    {
        ...
        Cell(0, _metricActive, L("active accounts"), GlyphUser);
        Cell(1, _metricAdmins, L("administrators"), GlyphAdmin);
        Cell(2, _metricRestricted, L("with limited hours"), GlyphClock);
    ...
    private void UpdateSummary()
    {
        ...
        _you.Text = me is null
            ? L("You're signed in with a domain or Microsoft Entra account: only local accounts are managed here.")
            : me.IsAdmin
                ? L("You're using “{0}”, an administrator account. For everyday use, a standard account limits the damage malware can do.", me.Name)
                : L("You're using “{0}”, a standard account: changes will require an administrator's password.", me.Name);
    }
    ...
    private WrapPanel BuildNavBar()
    {
        ...
        Add(L("Accounts"), GlyphUser, () => _accounts.Heading);
        Add(LC("short form", "Sign-in hours"), GlyphClock, () => _hours.Heading);
        Add(L("Restrictions"),GlyphBlock, () => _restrictions.Heading);
        Add(L("Parental controls"), GlyphFamily, () => _familyHeading);
        Add(L("Sign-in and lock"), GlyphLock, () => _signInHeading);
        return bar;
    }
    ...
    private Border BuildFamilyCard()
    {
        ...
        text.Children.Add(Text(L("Microsoft Family"), "Pp.CardTitle"));
        var body = Caption(
            L("Microsoft parental controls let you manage, from your phone or family.microsoft.com: daily screen time, limits per app and per game, web filtering in Microsoft Edge, purchase approval and activity reports. The parent and the child each need a Microsoft account, and the child must sign in to this PC with theirs."));
        body.Margin = new Thickness(0, 3, 0, 0);
        text.Children.Add(body);
        var open = MakeButton(L("Open Family in Settings"), GlyphOpen, "Pp.AccentButton", (_, _) => OpenUri("ms-settings:family-group"));
        ...
        root.Children.Add(Text(L("Tips for a family PC"), "Pp.CardTitle"));
        ...
        Tip(GlyphUser, L("A standard account for each child"),
            L("It can't install software or change Windows, and the sign-in hours and restrictions above apply to it."),
            MakeButton(L("Create an account"), GlyphAdd, "Pp.Button", async (_, _) => await _accounts.CreateAsync()));
        var dns = PageButton(L("Network"), "network", null);
        Tip("", L("A family DNS"),
            L("Free DNS resolvers (Cloudflare for Families, CleanBrowsing…) block adult sites across the whole PC, whatever the browser."), dns is null ? [] : [dns]);
        var guided = PageButton(L("Guided access"), "guided", null);
        var kiosk = PageButton(L("Kiosk"), "kiosk", null);
        Tip("", L("Guided access or kiosk mode"),
            L("For a young child: limit the session to a single full-screen app, with no access to the rest of the PC."),
            [.. new[] { guided, kiosk }.OfType<Button>()]);
        Tip(GlyphClock, L("Sign-in hours without a Microsoft account"),
            L("The sign-in hours on this page work with a plain local account, without an internet connection."),
            MakeButton(L("Set up"), null, "Pp.Button", (_, _) => ScrollTo(_hours.Heading)));
    ...
    [
        (3, L("After 3 failed attempts")), (5, L("After 5 failed attempts")), (10, L("After 10 attempts (Windows 11)")),
        (15, L("After 15 failed attempts")), (20, L("After 20 failed attempts")), (30, L("After 30 failed attempts")),
        (50, L("After 50 failed attempts")), (0, L("Never (not recommended)")),
    ];
    ...
    private Border BuildLockoutCard()
    {
        ...
        System.Windows.Automation.AutomationProperties.SetName(_lockoutChoice, L("Lockout threshold"));
        ...
        s.Children.Add(SettingRow(GlyphLock, L("Lockout after wrong passwords"),
            L("Temporarily locks a local account after several wrong passwords in a row: protects against anyone trying to guess a password or code on the PC."), controls,
            AppHost.Profile.IsDomainJoined ? L("Domain-joined PC: the domain policy overrides this local setting.") : null));
    ...
    private async Task LoadLockoutAsync()
    {
        ...
        if (lockout is null)
        {
            _lockoutState.Text = L("Couldn't read the current policy.");
            return;
        }
        var parts = new List<string>
        {
            lockout.Threshold == 0
                ? L("Currently: no lockout, attempts are unlimited.")
                : LP(lockout.Threshold, "Currently: lockout after {0} failed attempt, for {1} min.", "Currently: lockout after {0} failed attempts, for {1} min.", lockout.DurationMinutes),
        };
        if (pwd is not null)
            parts.Add(pwd.MinLength == 0 ? L("No minimum password length.") : LP(pwd.MinLength, "Passwords must have at least {0} character.", "Passwords must have at least {0} characters."));
        parts.Add(L("The Windows Hello PIN has its own protection against repeated attempts."));
        ...
        if (index < 0)
        {
            _lockoutChoice.Items.Insert(0, new ComboBoxItem { Content = LP(lockout.Threshold, "After {0} attempt (current)", "After {0} attempts (current)"), Tag = lockout.Threshold, IsEnabled = false });
            index = 0;
        }
    ...
    private async Task ApplyLockoutAsync()
    {
        ...
        if (value == 0 && !await AppHost.Dialogs.ConfirmAsync(L("Turn off lockout"),
                L("Without lockout, someone can try as many passwords as they want on this PC. Continue?"), L("Disable"), danger: true))
            return;
        _lockoutApply.IsEnabled = false;
        _lockoutChoice.IsEnabled = false;
    ...
    private static UIElement BuildSecurityLink()
    {
        ...
        var go = MakeButton(L("Open in Security"), null, "Pp.Button", (_, _) => TryNavigate("security", "tweak:security.logon.cad"));
        return Card(SettingRow("", L("Ctrl+Alt+Del before sign-in"),
            L("Requiring the secure sequence before entering a password is set on the Security page, along with the other sign-in protections."), go), new Thickness(16, 10, 16, 10));
    }
}
