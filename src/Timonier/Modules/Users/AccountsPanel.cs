...
internal sealed class AccountsPanel : StackPanel
{
    ...
    {
        _create = MakeButton(L("Nouveau compte"),GlyphAdd, "Pp.AccentButton", async (_, _) => await CreateAsync());
        _refresh = MakeButton(L("Actualiser"), GlyphRefresh, "Pp.SubtleButton", (_, _) => RefreshRequested?.Invoke());
        _refresh.ToolTip = L("Relire la liste des comptes");
        Children.Add(SectionHeader(L("Comptes"), out var heading, _busyBar, _refresh, _create));
        Heading = heading;
        ...
        System.Windows.Automation.AutomationProperties.SetName(_showSystem, L("Afficher les comptes techniques"));
        ...
        var label = Caption(L("Afficher les comptes techniques de Windows"));
    ...
    public void ShowLoading()
    {
        ...
        var t = Caption(L("Lecture des comptes locaux…"));
    ...
    private void Render()
    {
        ...
        {
            _list.Children.Add(EmptyState(GlyphWarning, L("Impossible de lire les comptes locaux"), _error ?? L("Erreur inconnue."),
                MakeButton(L("Réessayer"), GlyphRefresh, "Pp.Button", (_, _) => RefreshRequested?.Invoke())));
            return;
        }
        ...
        if (visible.Count == 0)
            _list.Children.Add(EmptyState(GlyphUser, L("Aucun compte local"), L("Ce PC n'utilise peut-être que des comptes de domaine ou Microsoft Entra.")));
    }
    ...
    private Border BuildCard(LocalAccount a, int enabledAdmins)
    {
        ...
        if (a.IsCurrent) titleRow.Children.Add(Badge(L("Vous"), "Pp.Info"));
        titleRow.Children.Add(a.IsAdmin ? Badge(L("Administrateur"), "Pp.Warning") : Badge(LC("account type", "Standard"), "Pp.Success"));
        if (!a.Enabled) titleRow.Children.Add(Badge(L("Désactivé"), "Pp.Neutral"));
        if (a.LockedOut) titleRow.Children.Add(Badge(L("Verrouillé"), "Pp.Danger"));
        if (a.MicrosoftAccount is not null) titleRow.Children.Add(Badge(L("Compte Microsoft"), "Pp.Info"));
        if (a.IsBuiltIn) titleRow.Children.Add(Badge(L("Intégré à Windows"), "Pp.Neutral"));
        else if (a.IsSystemAccount) titleRow.Children.Add(Badge(L("Reliquat d'installation"),"Pp.Neutral"));
        if (a.HasLogonRestriction) titleRow.Children.Add(Badge(L("Horaires limités"), "Pp.Info"));
        if (a.Enabled && a.PasswordNotRequired && a.MicrosoftAccount is null) titleRow.Children.Add(Badge(L("Mot de passe vide autorisé"), "Pp.Warning"));
        body.Children.Add(titleRow);

        var details = new List<string>();
        if (a.MicrosoftAccount is not null) details.Add(a.MicrosoftAccount);
        details.Add(a.LastLogon is { } ll ? L("Dernière connexion {0}", Format.Ago(ll)) : L("Jamais connecté"));
        if (a.PasswordLastSet is { } ps) details.Add(L("mot de passe défini le {0}", Format.Day(ps)));
    ...
    {
        500 => L("Compte administrateur caché créé par Windows, sans invite UAC : à laisser désactivé."),
        501 => L("Compte Invité : permettrait d'ouvrir une session sans mot de passe. À laisser désactivé."),
        503 => L("Compte technique utilisé par Windows (DefaultAccount)."),
        504 => L("Compte technique de Microsoft Defender Application Guard."),
        _ when LocalAccounts.IsSetupLeftover(a.Name) => L("Compte temporaire créé pendant l'installation de Windows ; il peut être supprimé."),
        _ => null,
    };
    ...
    private WrapPanel BuildActions(LocalAccount a, int enabledAdmins)
    {
        ...
        var sessionReason = L("Impossible sur le compte avec lequel vous êtes connecté.");
        var lastAdminReason = L("C'est le dernier administrateur actif de ce PC.");
        ...
        if (a.IsCurrent)
        {
            Add(L("Options de connexion"), GlyphKey, () => { OpenUri("ms-settings:signinoptions"); return Task.CompletedTask; })
                .ToolTip = L("Changer votre mot de passe, votre code PIN ou Windows Hello");
        }
        else if (a.Rid != 501)
        {
            string? reason = a.MicrosoftAccount is not null ? L("Compte Microsoft : le mot de passe se change sur account.microsoft.com.") : null;
            Add(L("Mot de passe…"), GlyphKey, () => ResetPasswordAsync(a), reason);
        }

        if (!a.IsBuiltIn && a.MicrosoftAccount is null)
            Add(L("Nom affiché…"), "", () => RenameAsync(a));

        if (!a.IsBuiltIn)
        {
            if (a.IsAdmin)
                Add(L("Rendre standard"), GlyphUser, () => SetTypeAsync(a, admin: false), a.IsCurrent ? sessionReason : lastAdmin ? lastAdminReason : null);
            else
                Add(L("Rendre administrateur"),GlyphAdmin, () => SetTypeAsync(a, admin: true));
        }

        if (a.LockedOut)
            Add(L("Déverrouiller"), "", () => SetEnabledAsync(a, true));
        if (a.Enabled)
            Add(L("Désactiver"), "", () => SetEnabledAsync(a, false), a.IsCurrent ? sessionReason : lastAdmin ? lastAdminReason : null);
        else if (!a.IsBuiltInAdministrator && !a.IsGuest)
            Add(L("Activer"), "", () => SetEnabledAsync(a, true));

        if (!a.IsBuiltIn)
            Add(L("Supprimer"), GlyphDelete, () => DeleteAsync(a), a.IsCurrent ? sessionReason : lastAdmin ? lastAdminReason : null);

        if (!a.IsAdmin && !a.IsCurrent && !a.IsBuiltIn && a.Enabled)
        {
            Add(L("Horaires"), GlyphClock, () => { NavigateRequested?.Invoke("section:hours", a.Sid); return Task.CompletedTask; });
    ...
    {
        if (!enable && !await AppHost.Dialogs.ConfirmAsync(L("Désactiver le compte"),
                L("« {0} » ne pourra plus ouvrir de session. Ses fichiers et ses réglages sont conservés ; vous pourrez le réactiver à tout moment.", a.Name),
                L("Désactiver"), danger: true))
            return;
    ...
    {
        if (!admin && !await AppHost.Dialogs.ConfirmAsync(L("Rendre le compte standard"),
                L("« {0} » ne pourra plus installer de logiciels ni modifier les réglages du PC sans le mot de passe d'un administrateur. Le changement prend effet à sa prochaine ouverture de session.", a.Name), L("Rendre standard")))
            return;
        if (admin && !await AppHost.Dialogs.ConfirmAsync(L("Rendre le compte administrateur"),
                L("« {0} » aura un contrôle total sur ce PC : installer des logiciels, modifier les autres comptes, retirer les restrictions. Windows vous demandera de confirmer dans une fenêtre administrateur.", a.Name), L("Continuer"), danger: true))
            return;
    ...
    {
        if (!await AppHost.Dialogs.ConfirmAsync(L("Supprimer le compte"),
                L("Le compte « {0} » sera supprimé et ne pourra plus ouvrir de session. Son dossier de profil (documents, bureau, images) reste sur le disque : vous pourrez le récupérer ou le supprimer ensuite depuis « Profils des utilisateurs ».\n\nUne confirmation vous sera encore demandée dans une fenêtre administrateur.", a.Name), L("Supprimer"), danger: true))
            return;
    ...
    {
        var value = await AppHost.Dialogs.PromptAsync(L("Nom affiché"),
            L("Nom complet affiché pour « {0} » (écran de connexion, menu Démarrer). Le nom du compte lui-même ne change pas.", a.Name),
    ...
    private async Task ResetPasswordAsync(LocalAccount a)
    {
        ...
        content.Children.Add(Warn(L("Réinitialiser un mot de passe fait perdre à ce compte l'accès à ses fichiers chiffrés (EFS), à ses mots de passe enregistrés par Windows et à ses certificats. Préférez que la personne le change elle-même si elle s'en souvient.")));
        content.Children.Add(Caption(L("Nouveau mot de passe")));
        content.Children.Add(first);
        content.Children.Add(Caption(L("Confirmer le mot de passe")));
        content.Children.Add(second);
        content.Children.Add(error);
        try
        {
            while (await AppHost.Dialogs.ShowAsync(L("Mot de passe de « {0} »", a.Name), content, L("Réinitialiser"), L("Annuler"), danger: true))
            {
                string? problem = first.Password.Length == 0 ? L("Saisissez un mot de passe.")
                    : first.Password != second.Password ? L("Les deux mots de passe ne correspondent pas.")
                    : first.Password.Length > AccountParams.MaxPassword ? L("Mot de passe trop long.")
                    : null;
                if (problem is null)
                {
    ...
    public async Task CreateAsync()
    {
        ...
        standard.Content = RadioLabel(L("Standard (recommandé)"), L("Pour un enfant ou l'usage quotidien : ne peut ni installer de logiciels ni modifier le PC."));
        var admin = new RadioButton { GroupName = "tmn-users-type", Margin = new Thickness(0, 4, 0, 4) };
        admin.Content = RadioLabel(L("Administrateur"), L("Contrôle total du PC, y compris des autres comptes."));
        var error = Text("", "Pp.Caption").Themed(TextBlock.ForegroundProperty, "Pp.Danger");
        error.Visibility = Visibility.Collapsed;
        error.Margin = new Thickness(0, 8, 0, 0);

        var content = new StackPanel { Width = 440 };
        content.Children.Add(Caption(L("Nom du compte (1 à 20 caractères : lettres, chiffres, espace, . _ -)")));
        content.Children.Add(name);
        content.Children.Add(Caption(L("Nom complet (facultatif, affiché à la connexion)")));
        content.Children.Add(fullName);
        content.Children.Add(Caption(L("Mot de passe")));
        content.Children.Add(pwd);
        content.Children.Add(Caption(L("Confirmer le mot de passe")));
        content.Children.Add(pwd2);
        var hint = Caption(L("Laissé vide, n'importe qui pourra ouvrir ce compte. La personne pourra ensuite définir un code PIN."));
        hint.FontStyle = FontStyles.Italic;
        content.Children.Add(hint);
        var typeTitle = Caption(L("Type de compte"));
        ...
        try
        {
            while (await AppHost.Dialogs.ShowAsync(L("Nouveau compte local"),content, L("Créer"), L("Annuler")))
            {
                string? problem = null;
                try
                {
                    ...
                    if (pwd.Password != pwd2.Password) problem = L("Les deux mots de passe ne correspondent pas.");
                    else if (_accounts?.Any(a => string.Equals(a.Name, name.Text.Trim(), StringComparison.OrdinalIgnoreCase)) == true)
                        problem = L("Un compte porte déjà ce nom.");
                }
                catch (ValidationException ex) { problem = ex.Message; }

                if (problem is null)
                {
                    if (pwd.Password.Length == 0 && !await AppHost.Dialogs.ConfirmAsync(L("Compte sans mot de passe"),
                            L("Sans mot de passe, toute personne ayant accès au PC pourra ouvrir ce compte et lire ses fichiers. Créer quand même ?"),
                            L("Créer sans mot de passe"), L("Revenir")))
                    {
                        content = Detach(content);
                        continue;
                    }
    ...
    private static WrapPanel BuildToolsRow()
    {
        ...
        Add(L("Autres utilisateurs"),"", () => OpenUri("ms-settings:otherusers"), L("Paramètres › Comptes › Autres utilisateurs (comptes Microsoft, comptes professionnels)"));
        Add(L("Options de connexion"), GlyphKey, () => OpenUri("ms-settings:signinoptions"), L("Code PIN, Windows Hello, mot de passe de votre compte"));
        Add(L("Profils des utilisateurs"), "", () => Launch(SystemTool.Rundll32, "sysdm.cpl,EditUserProfiles"), L("Supprimer le dossier de profil d'un ancien compte"));
        Add("netplwiz", "", () => Launch(SystemTool.Netplwiz), L("Outil classique « Comptes d'utilisateurs »"));
        return row;
    }
}
