...
public sealed class DashboardPage : UserControl, INavigationAware
{
    ...
    private static Grid SpecGrid(SystemProfile p, bool hardware)
    {
        var items = new List<UIElement>
        {
            Spec("", "Windows",
                ...
                L("Version {0} · build {1}.{2}", p.DisplayVersion, p.Build, p.Ubr) + (string.IsNullOrEmpty(p.Architecture) ? "" : " · " + p.Architecture)),
        };
        if (!hardware)
        {
    ...
    private void RenderHealth()
    {
        ...
        var ordered = list
            ...
            .ThenBy(x => x.Check.Title, StringComparer.Create(Loc.Culture, true))
            .ToList();
    ...
    private void RenderRecommendations(List<TweakDefinition> mismatches, int analysed)
    {
        ...
        var number = DashUi.Text(mismatches.Count.ToString(Loc.Culture), "Pp.Metric", 30);
        ...
        var groups = mismatches
            ...
            .ThenBy(g => g.Category?.Title ?? g.Id, StringComparer.Create(Loc.Culture, true))
            .ToList();
        var first = true;
    ...
    private FrameworkElement RecoRow(CategoryInfo? category, string id, List<TweakDefinition> items)
    {
        ...
        var count = DashUi.Badge(items.Count.ToString(Loc.Culture), "Pp.AccentText", "Pp.AccentSubtle");
    ...
    {
        var s = new StackPanel();
        s.Children.Add(DashUi.Text(value.ToString("N0", Loc.Culture), "Pp.Metric", 26));
