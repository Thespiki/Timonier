...
internal static class DashboardQuickActions
{
    ...
    {
        var ok = await AppHost.Dialogs.ConfirmAsync("Redémarrer l'Explorateur Windows ?",
            "La barre des tâches et le bureau disparaissent une ou deux secondes puis reviennent. Les fenêtres de l'Explorateur " +
            "de fichiers ouvertes seront fermées ; vos applications et documents ne sont pas touchés.",
            "Redémarrer l'Explorateur");
        if (!ok) return;
        try
        {
            await SystemEffects.RestartExplorerAsync();
            AppHost.Toasts.Show("Explorateur Windows redémarré.", ToastKind.Success);
        }
        catch (Exception ex)
        {
            Log.Error("Dashboard", "redémarrage de l'Explorateur", ex);
            AppHost.Toasts.Show("Impossible de redémarrer l'Explorateur : " + ex.Message, ToastKind.Error);
        }
    }
    ...
    public static Task OpenTaskManagerAsync()
    {
        ...
        catch (Win32Exception ex) when (ex.NativeErrorCode == 740)
        {
            ...
            catch (Exception inner) { Fail("le Gestionnaire des tâches", inner); }
        }
        catch (Exception ex) { Fail("le Gestionnaire des tâches", ex); }
    ...
    public static Task OpenSettingsAsync()
    {
        ...
        catch (Exception ex) { Fail("les Paramètres Windows", ex); }
        return Task.CompletedTask;
    }
    ...
    public static Task LockAsync()
    {
        if (!DashNative.LockWorkStation())
            AppHost.Toasts.Show("Windows a refusé le verrouillage de la session.", ToastKind.Warning);
        return Task.CompletedTask;
    }
    ...
    private static void Fail(string what, Exception ex)
    {
        ...
        AppHost.Toasts.Show($"Impossible d'ouvrir {what} : {ex.Message}", ToastKind.Error);
    }
}
