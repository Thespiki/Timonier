...
public sealed class JournalPage : UserControl, INavigationAware
{
    ...
    public JournalPage()
    {
        ...
        var stack = PageScaffold.Create(this, L("Change history"),
            L("Everything Timonier has changed on this PC, with the previous state so you can go back."), AppPagesModule.JournalGlyph);

        // Résumé + annulations groupées
        _undoSession = AppUi.Button(L("Undo session"), "", "Pp.Button", async (_, _) => await UndoManyAsync(session: true));
        _undoSession.ToolTip = L("Undoes the changes made since Timonier was opened, from newest to oldest.");
        _undoToday = AppUi.Button(L("Undo today"), "", "Pp.Button", async (_, _) => await UndoManyAsync(session: false));
        _undoToday.ToolTip = L("Undoes all changes made today, from newest to oldest.");
        ...
        // Barre d'outils
        _search = AppUi.SearchBox(L("Search history (setting, value, note…)"), q => { _query = q; _limit = PageSize; Render(); });
        _export = AppUi.Button(L("Export (CSV)"), "", "Pp.Button", async (_, _) => await ExportAsync());
        _export.ToolTip = L("Saves all history entries to a CSV file (UTF-8, “;” separator) that you can open in Excel.");
        _clear = AppUi.Button(L("Clear user history"), "", "Pp.SubtleButton", async (_, _) => await ClearUserJournalAsync());
        ...
        stack.Children.Add(PageScaffold.InfoBar(
            L("Changes made without administrator rights are saved in your profile (journal.json). Those made by the admin session are saved in HKLM\\SOFTWARE\\Timonier\\Journal, which only administrators can change, so Timonier can only undo what it actually recorded. One-time actions (system tools) can't be undone."),
            ""));
    ...
    private async Task ReloadAsync()
    {
        ...
        if (_all.Count == 0)
        {
            ...
            _list.Children.Add(AppUi.StateCard("", L("Reading history…"), busy: true));
        }
        try
        {
        ...
        catch (Exception ex)
        {
            ...
            _list.Children.Clear();
            var card = AppUi.StateCard("", L("History couldn't be read"), ex.Message);
            var retry = AppUi.Button(L("Try again"), "", "Pp.Button", (_, _) => _ = ReloadAsync());
    ...
    private void Render()
    {
        ...
        if (_all.Count == 0)
        {
            _list.Children.Add(AppUi.StateCard(AppPagesModule.JournalGlyph, L("No changes yet"),
                L("When you apply a setting or an action with Timonier, the change appears here with the previous state, so you can go back at any time. Nothing is changed without your consent.")));
            return;
        }
        ...
        if (items.Count == 0)
        {
            _list.Children.Add(AppUi.StateCard("", L("No matching entries"),
                _query.Length > 0 ? L("No results for “{0}” with this filter.", _query) : L("No entries in this category.")));
            return;
        }
        ...
        foreach (var day in shown.GroupBy(e => e.At.LocalDateTime.Date))
        {
            ...
            var countText = AppUi.Caption(LP(count, "{0} change", "{0} changes"));
        ...
        if (items.Count > shown.Count)
        {
            ...
            var more = AppUi.Button(LP(remaining, "Show more ({0} change left)", "Show more ({0} changes left)"), "", "Pp.Button",
    ...
    private void UpdateSummary()
    {
        ...
        Metric(_all.Count.ToString(Culture), L("total"));
        Metric(_all.Count(e => e.CanUndo).ToString(Culture), LC("journal metric", "undoable"));
        Metric(today.ToString(Culture), LC("journal metric", "today"));
        Metric(session.ToString(Culture), LC("journal metric", "since launch"));
    }
    ...
    private FrameworkElement BuildRow(JournalEntry e)
    {
        ...
        time.ToolTip = L("{0} at {1}", e.At.LocalDateTime.ToString("D", Culture), e.At.LocalDateTime.ToString("T", Culture));
        g.Children.Add(time);
        ...
        AddBadge(e.Machine ? AppUi.Badge(LC("badge", "Admin"), "Info", "") : AppUi.Badge(L("User"), "Neutral", ""));
        if (e.Undone)
        {
            var at = e.UndoneAt?.LocalDateTime;
            AddBadge(AppUi.Badge(at is { } d ? L("Undone on {0}", Format.Date(d)) : L("Undone"), "Success", ""));
        }
        else if (!e.CanUndo)
            AddBadge(AppUi.Badge(LC("badge", "Can't be undone"), "Warning", ""));
        if (e.Machine && !string.IsNullOrEmpty(e.UserSid) && !string.IsNullOrEmpty(AppHost.Profile.UserSid)
            ...
            AddBadge(AppUi.Badge(L("Other account"), "Neutral", ""));
        ...
        if (AppHost.Registry.GetTweak(e.SourceId) is { } tweak)
        {
            ...
            open.ToolTip = L("Open this setting");
            System.Windows.Automation.AutomationProperties.SetName(open, L("Open this setting"));
            open.Width = 34;
        ...
        if (e.CanUndo)
        {
            var undo = AppUi.Button(L("Undo"), "", "Pp.Button");
            undo.Margin = new Thickness(6, 0, 0, 0);
            undo.ToolTip = e.Machine ? L("Undoes through the admin session (a UAC prompt may appear).") : L("Restores the previous state.");
    ...
    private static string DayLabel(DateTime day)
    {
        ...
        if (day == DateTime.Today) return L("Today — {0}", full);
        if (day == DateTime.Today.AddDays(-1)) return L("Yesterday — {0}", full);
    ...
    {
        if (_busy) return;
        SetBusy(true, L("Undoing: {0}…", entry.Title));
        AppUi.SetButtonText(button, L("Undoing…"));
        try
        {
    ...
    private async Task UndoManyAsync(bool session)
    {
        ...
        if (items.Count == 0)
        {
            AppHost.Toasts.Show(session ? L("No changes to undo since Timonier was opened.") : L("No changes to undo today."),
                ToastKind.Info);
            return;
        }
        ...
        content.Children.Add(AppUi.Text(session
            ? LP(items.Count, "The following change, made since Timonier was opened, will be undone:",
                "The following {0} changes, made since Timonier was opened, will be undone from newest to oldest:")
            : LP(items.Count, "The following change, made today, will be undone:",
                "The following {0} changes, made today, will be undone from newest to oldest:")));
        ...
        if (items.Count > 12) list.Children.Add(AppUi.Caption(LP(items.Count - 12, "… and {0} more.", "… and {0} more.")));
        content.Children.Add(list);
        if (items.Any(e => e.Machine))
        {
            var admin = AppUi.Caption(L("Some changes were made with administrator rights: a UAC prompt may appear. If you decline it, the undo stops there and the rest is kept."));
            admin.Margin = new Thickness(0, 12, 0, 0);
            content.Children.Add(admin);
        }

        if (!await AppHost.Dialogs.ShowAsync(session ? L("Undo this session's changes") : L("Undo today's changes"),
                content, L("Undo all"), L("Keep")))
            return;
        ...
        try
        {
            ...
            {
                var e = items[i];
                SetBusy(true, L("Undoing {0} of {1}: {2}…", i + 1, items.Count, e.Title));
        ...
        if (failures.Count == 0 && !stopped)
        {
            AppHost.Toasts.ShowOutcome(new ApplyOutcome(true,
                LP(done, "{0} change undone.", "{0} changes undone."), effect));
            return;
        }
        // Phrases complètes juxtaposées (chacune porte son propre pluriel).
        var parts = new List<string> { LP(done, "{0} change undone.", "{0} changes undone.") };
        if (stopped) parts.Add(LP(remaining, "{0} change not processed (undo interrupted).", "{0} changes not processed (undo interrupted)."));
        if (failures.Count > 0) parts.Add(LP(failures.Count, "{0} change failed.", "{0} changes failed."));
        ...
        if (failures.Count > 0)
            await AppHost.Dialogs.AlertAsync(L("Failed undos"), string.Join(Environment.NewLine, failures.Take(10)));
    }
    ...
    private async Task ExportAsync()
    {
        ...
        {
            Title = L("Export Timonier history"),
            FileName = L("Timonier - history {0}.csv", DateTime.Now.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)),
            DefaultExt = ".csv",
        ...
        try
        {
            ...
            AppHost.Toasts.Show(LP(rows.Count, "History exported: {0} entry.", "History exported: {0} entries."), ToastKind.Success, L("Open folder"), () =>
            {
                try { if (Path.GetDirectoryName(path) is { } dir) ProcessRunner.OpenFolder(dir); }
                catch (Exception ex) { AppHost.Toasts.Show(L("Couldn't open the folder: {0}", ex.Message), ToastKind.Error); }
            });
        }
        catch (Exception ex)
        {
    ...
    private static string BuildCsv(IEnumerable<JournalEntry> rows)
    {
        ...
        sb.AppendLine(L("Date;Time;Title;ID;Change;Scope;Undoable;Undone on;Note"));
        foreach (var e in rows)
        {
            ...
            sb.Append(Esc(at.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture))).Append(';')
              ...
              .Append(e.Machine ? LC("badge", "Admin") : L("User")).Append(';')
              .Append(e.CanUndo ? L("Yes") : L("No")).Append(';')
    ...
    private async Task ClearUserJournalAsync()
    {
        ...
        message.Append(LP(user.Count, "The user history entry will be cleared.", "The {0} user history entries will be cleared."));
        if (undoable > 0)
            message.Append(' ').Append(LP(undoable,
                "{0} of them can still be undone: the change will remain in place, but Timonier will no longer be able to undo it automatically.",
                "{0} of them can still be undone: the changes will remain in place, but Timonier will no longer be able to undo them automatically."));
        message.Append("\n\n");
        message.Append(machine > 0
            ? LP(machine,
                "The admin history ({0} entry) is stored in HKLM\\SOFTWARE\\Timonier\\Journal: it's managed by the admin session, which keeps the 500 most recent entries, and can't be changed from the interface without administrator rights.",
                "The admin history ({0} entries) is stored in HKLM\\SOFTWARE\\Timonier\\Journal: it's managed by the admin session, which keeps the 500 most recent entries, and can't be changed from the interface without administrator rights.")
            : L("The admin history is stored in HKLM\\SOFTWARE\\Timonier\\Journal: it's managed by the admin session, which keeps the 500 most recent entries, and can't be changed from the interface without administrator rights."));
        if (!await AppHost.Dialogs.ConfirmAsync(L("Clear user history?"), message.ToString(), L("Clear"), L("Undo"), danger: true))
            return;
        try
        {
            await Task.Run(JournalWriter.UserStore.Clear);
            AppHost.Toasts.Show(L("User history cleared."), ToastKind.Success);
        }
        catch (Exception ex)
        {
            Log.Error("AppPages", "vidage du journal", ex);
            AppHost.Toasts.Show(L("Couldn't clear history: {0}", ex.Message), ToastKind.Error);
        }
        await ReloadAsync();
    }
}
