...
public static class DeviceInventory
{
    ...
    private static DeviceEntry? ToEntry(Dictionary<string, object?> r)
    {
        ...
        if (string.IsNullOrWhiteSpace(name)) name = L("Unknown device");
...
/// <summary>
/// Mémoire des périphériques désactivés par Timonier (préférences de l'interface) : permet de proposer « Réactiver »
