...
public static class TextNormalizer
{
    ...
    public static string[] Tokens(string text, bool keepStopWords = false)
    {
        ...
        var parts = n.Split(' ', StringSplitOptions.RemoveEmptyEntries).SelectMany(Segment);
        var tokens = keepStopWords ? parts : parts.Where(p => !StopWords.Contains(p));
        // Pluriels simples : « parametres » ≈ « parametre ».
        return [.. tokens.Select(t => t.Length > 4 && t.EndsWith('s') && !t.EndsWith("ss") ? t[..^1] : t).Distinct()];
    }

    /// <summary>
    /// Chinois, japonais et thaï s'écrivent sans espaces : leurs suites de caractères sont découpées en paires
    /// chevauchantes (« 网络设置 » → « 网络 », « 络设 », « 设置 »), ce qui permet de trouver un mot au milieu d'un titre.
    /// </summary>
    private static IEnumerable<string> Segment(string part)
    {
        if (!part.Any(IsDenseScript)) { yield return part; yield break; }
        var i = 0;
        while (i < part.Length)
        {
            var dense = IsDenseScript(part[i]);
            var j = i;
            while (j < part.Length && IsDenseScript(part[j]) == dense) j++;
            if (!dense || j - i == 1) yield return part[i..j];
            else for (var k = i; k < j - 1; k++) yield return part.Substring(k, 2);
            i = j;
        }
    }

    private static bool IsDenseScript(char c) =>
        c is >= '぀' and <= 'ヿ'                                        // hiragana, katakana
            or >= '㐀' and <= '䶿' or >= '一' and <= '鿿'       // idéogrammes CJC
            or >= '豈' and <= '﫿'
            or >= '฀' and <= '๿';                                     // thaï
}
