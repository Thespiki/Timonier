...
public sealed class TweakEngine(ModuleRegistry registry, BrokerClient broker, SystemProfile profile)
{
    ...
    public async Task<ApplyOutcome> ApplyAsync(TweakDefinition tweak, string optionKey, CancellationToken ct = default)
    {
        if (tweak.GetOption(optionKey) is not { } option)
            return new ApplyOutcome(false, L("Unknown option “{0}”.", optionKey));
        ...
        IProgress<string>? progress = null, CancellationToken ct = default)
    {
        ...
        if (handler is null) return new ApplyOutcome(false, L("Unknown action: {0}", actionId));
    ...
    {
        if (!entry.CanUndo) return new ApplyOutcome(false, L("This change can't be undone automatically."));
        try
        {
            ApplyOutcome outcome;
            if (entry.Machine)
            ...
            else
            {
                ...
                outcome = errors.Count == 0
                    ? new ApplyOutcome(true, L("Undone: {0}", entry.Title), EffectOf(entry.SourceId))
                    : new ApplyOutcome(false, L("Partially undone: {0}", string.Join(" ; ", errors)));
            }
    ...
    {
        UnauthorizedAccessException => L("Access denied. {0}", ex.Message),
        Win32Exception { NativeErrorCode: 5 } => L("Access denied by Windows (protected item)."),
        System.Security.SecurityException => L("Access denied by Windows (protected item)."),
        _ => ex.Message,
    };
}
