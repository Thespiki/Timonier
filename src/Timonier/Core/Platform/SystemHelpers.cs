    ...
    public static RegistryKey OpenRoot(RegHive hive, string? userSid, bool writable = true)
    {
        return hive switch
        {
            ...
            RegHive.CurrentUser when !string.IsNullOrEmpty(userSid) =>
                ...
                ?? throw new InvalidOperationException(L("User hive not loaded: {0}", userSid)),
...
public static partial class ServiceConfig
{
    ...
    {
        if (!IsValidName(name)) throw new ArgumentException(L("Invalid service name: {0}", name));
...
public static class TaskSchedulerHelper
{
    ...
    {
        if (!IsValidPath(path)) throw new ArgumentException(L("Invalid task path: {0}", path));
        ...
        try
        {
            foreach (dynamic t in folder.GetTasks(1 /* TASK_ENUM_HIDDEN */))
            {
                ...
                try
                {
                    ...
                    var state = (int)t.State switch { 1 => L("Disabled"), 2 => LC("short form", "Queued"), 3 => L("Ready"), 4 => L("Running"), _ => L("Unknown") };
