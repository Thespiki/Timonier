...
/// <summary>Demande de capture : fichier PNG, page à afficher, thème (light/dark), paramètre de navigation, défilement vertical, langue.</summary>
    ...
    public static async Task RunAsync(App app, MainWindow window, CaptureRequest request)
    {
        try
        {
            ...
            using (var dc = visual.RenderOpen())
            {
                ...
                // Interface de droite à gauche : le VisualBrush reproduit le miroir interne sans la compensation de la
                // fenêtre ; on le retourne pour obtenir l'image réellement affichée.
                if (window.FlowDirection == FlowDirection.RightToLeft)
                    dc.PushTransform(new MatrixTransform(-1, 0, 0, 1, root.ActualWidth, 0));
